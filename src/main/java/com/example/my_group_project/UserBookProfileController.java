...
public class UserBookProfileController extends UserMenuController {
    ...
    void moreBook(List<Book> books) throws IOException {
        if(books.size() == 0 ) {
            System.out.print("no books recommend");
        }
        else {
            for (int i = 0; i < 10; i++) {
                ...
                if (imageUrl != null && !imageUrl.isEmpty()) {
                    ...
                    // Set a default image if the image URL is invalid or null
                    image.setImage(new Image("C:\\Users\\Dell\\oop\\Library-Management-System-web\\src\\main\\resources\\picture\\atLogInPage.png"));
                }
        ...
        if(currentBook == null) {
            try {
                ...
                if(currentBook.getGenre() != null && !currentBook.getGenre().isEmpty()){
                    ...
                    System.out.print("khong co sach goi y");
                    //moreBook(recommendedBooks);
                }
    ...
    void borrowButtonOnAction(ActionEvent event) {
        ...
        UserHomeController.showIntro("Ban da muon thanh cong", BaseController.getMainStage());
    }
